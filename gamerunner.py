...
def run_game(asp, bots, visualizer=None, delay=0.2, max_wait=0.3, colored=True):
    ...
    while not (asp.is_terminal_state(state)):
        ...
        try:
            ...
            if True or visualizer:
                print("""Warning. Player %s took too long to decide on a move.
                    They will go UP this round.""" % (state.ptm + 1))
                free_space_timeout = len(bots[state.ptm].free_space) + len(bots[state.ptm].opponent_free_space)
                free_space_timeouts.append(free_space_timeout)
                print("Bot timed out, free space to search: {}".format(free_space_timeout))
            decision = "U"
...
class HelpMessage:
    ...
    MAX_WAIT = (
        ...
        they move up. Defaults to """
        + str(Argument_Defaults.MAX_WAIT)
    )
    BOTS = (
        '''which bot each player will use. Valid bot types include "user",
        "wall", "random". This argument takes in a sequnce of bot types,
    ...
    NO_IMAGE = """include this flag (with no arguments) to suppress the output of all
        board states and retun only final results"""
    MULTI_TEST = """Test this map (multi_test) times in a row. Useful if you want to see how
        randomized algorithms do on average. Tracks how many times each player won across all games."""
